...
В логе ретранслятора видно `push_sent` / `push_stale` и, при старте,
`push_tokens_on_disk`; в логе приложения — `push_register_ok`. Строка
`push_tokens_in_memory` или `push_tokens_disk_failed` означает, что том не
подключён: push будет работать до первого деплоя. Если в логе `push_register_unsupported`, устройство не
