...
export async function messageSend(input: {
  ...
  return serialize(async () => {
    ...
    if (value === null) {
      ...
      //
      // Отдаём ИМЕНА строк, а не сами строки. Журнал общий на процесс, и
      // очередь выше выстраивает только отправки: за время вызова в него
      // попадает и входящий трафик, у которого в `meta` лежат куски чужих DID
      // (`dm_rejected_non_contact`, `dm_blocked_no_implicit_contact`). Сырая
      // строка утащила бы их вызывающему — ровно то, ради чего сам файл
      // журнала лежит с правами 0600. Имя отказа диагностику даёт, чужие
      // адреса — нет.
      return refuse(
        'unknown',
        'ядро не отправило сообщение и не назвало причину; имена строк журнала за время вызова приложены',
        entries.map((e) => e.msg ?? 'unparsed').slice(-20)
      );
    }
