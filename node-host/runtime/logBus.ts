 ...
 *
 * Режим в `openSync` действует только на создание: если файл уже лежит —
 * остался от прежней сборки, где прав не ставили, или его подложили, — аргумент
 * молча игнорируется, и журнал с DID продолжает лежать открытым. Поэтому права
 * выставляются отдельно и на каждом запуске. Через дескриптор, а не через путь:
 * так права ложатся именно на тот файл, который мы открыли, а не на то, чем
 * путь успел стать после открытия.
 */
export function attachLogSink(file: string): void {
  try {
    ...
    try {
      ...
      /* на файловых системах без прав (Windows, сетевые шары) это ожидаемо */
    }
    fs.closeSync(fd);
  } catch {
