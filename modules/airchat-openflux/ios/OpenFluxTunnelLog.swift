 ...
 * неизвестно что, слой 2 — только URLSession, а веб-сокеты идут мимо обоих
 * предположений. Нарисовать в настройках зелёную галочку «туннель работает»
 * по факту «ядро поднялось» — это соврать: ядро может стоять поднятым, пока
 * весь трафик идёт напрямую. Поэтому показываем единственное, что является
 * доказательством: сколько соединений ядро приняло на свой SOCKS5.
 *
