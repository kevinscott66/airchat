...
describe('копий правила больше нет', () => {
  it('склонение по «n < 5» нигде не пишется заново', () => {
    ...
    // v4.32.864: и третья половина — та самая сокращённая запись, записанная
    // не функцией, а прямо в разметке. Сторож искал объявление, а тернарник
    // объявлением не является: в настройках два таких прожили незамеченными и
    // подписывали «21 шаблонов», «22 устройств». Ловится сравнение с пятёркой,
    // выбирающее между русскими словами; арифметике с числами оно не мешает.
    const SHORTCUT = /<\s*5\s*\?\s*'[^']*[\u0430-\u044f\u0451]/;
    const shortcuts = files
      .map((f) => ({ key: f.slice(SRC.length + 1), src: readFileSync(f, 'utf8') }))
      .filter((f) => f.key !== HOME && SHORTCUT.test(f.src))
      .map((f) => f.key);
