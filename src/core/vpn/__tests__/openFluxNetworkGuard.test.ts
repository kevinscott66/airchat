...
describe('границы: чего автоматика делать не должна', () => {
  ...
  it('на платформе без ядра не трогает ни туннель, ни транспорт', async () => {
    // Здесь именно web. Раньше стояло 'ios' — тогда ядра под iOS не было, и
    // проверка держалась на этом. Теперь iOS поддерживается наравне с Android
    // (openFluxAvailable в ui/platformCapabilities), так что платформой без ядра
    // остаётся только web, и проверять надо на ней.
