...
describe('форма модуля', () => {
  it('импорт ровно один — счёт и тексты проверяются отдельно от сети', () => {
    // v4.32.842: подпись предела («8 МБ») приехала сюда из uploadRoute, чтобы
    // отчёт и отказ рядом с ним называли одно число одними словами. Больше
