...
// Полифилл Buffer: им выводится имя файла кэша для дескриптора без blob-id, и
// повторять эту арифметику вручную значило бы разойтись с уже лежащими на диске
// файлами. sha256 — чистый расчёт, платформы не касается: им к имени файла
// подмешивается отпечаток ключа (см. blobCacheName).
...
/**
 * Потолок ПРИНИМАЕМОЙ base64-строки вложения — и с релея, и из облака.
 *
 * Открытые 8 МБ после шифрования и base64 дают ~10.7 млн символов, так что
 * запас есть; всё, что больше, — уже не наше вложение, а ответ произвольной
 * длины от того, кто отвечает. Buffer.from синхронная, и разворачивать её
 * результат в память до этой проверки нельзя.
 */
export const MAX_DOWNLOAD_B64_CHARS = 12_000_000;
...
/**
 * Разделитель между id вложения и отпечатком ключа в имени файла кэша.
 *
 * В id его не бывает: LAN-id — 32 шестнадцатеричных знака, токен с релея —
 * `[A-Za-z0-9._-]`, хеш адреса — шестнадцатеричный. Поэтому по первому такому
 * знаку имя разбирается однозначно (см. cacheFileBlobId).
 */
export const BLOB_CACHE_KEY_SEP = '~';

/**
 * Отпечаток ключа вложения — вторая половина имени файла кэша.
 *
 * v4.32.615. Имя файла выводилось из одного `i`, а `i` выбирает ОТПРАВИТЕЛЬ, и
 * попадание в кэш возвращало файл ДО расшифровки — то есть AEAD не проверялся
 * вовсе. Значит, знающий чужой `i` мог занять это имя своим содержимым: в
 * общей группе дескриптор виден всем участникам, так что подсмотреть `i` у
 * соседа и заранее прислать мне личным сообщением вложение под тем же именем —
 * обычное действие, а не подбор. Дальше история или снимок этого соседа
 * рисовались бы чужой картинкой, и подмену нечем было бы заметить.
 *
 * Ключ у каждого вложения свой и случайный, поэтому отпечаток в имени делает
 * попадание в кэш утверждением «этот файл расшифрован ЭТИМ ключом». Занять
 * имя, не зная ключа, больше нельзя. Заодно исчезает и старое совпадение имён
 * у дескрипторов без `i`: их id — первые 12 знаков адреса, и два разных адреса
 * с общим началом делили один файл.
 *
 * Сам ключ в имя класть нельзя: имена файлов видны там, где содержимое уже
 * защищено, а ключ отпирает ещё и копию шифротекста на релее.
 */
export function blobKeyFingerprint(k: string): string {
  return Buffer.from(sha256(Buffer.from(`airchat-blob-key-v1|${k}`, 'utf8')))
    .toString('hex')
    .slice(0, 16);
}

/**
 * Имя файла расшифрованного вложения в кэше: `airchat_media_<id>~<отпечаток>.<ext>`.
 *
 * Первая половина имени осталась прежней намеренно: по ней кэш сверяется с
 * живыми ссылками (blobCacheIdsIn сканирует текст сообщения, а не разбирает
 * его), и меняться ей нельзя — иначе расшифрованное вложение переставало бы
 * исчезать вместе со своим сообщением.
 */
export function blobCacheName(ref: BlobRef, ext: string): string | null {
  const id = blobCacheId(ref);
  if (!id) return null;
  return `${BLOB_CACHE_PREFIX}${id}${BLOB_CACHE_KEY_SEP}${blobKeyFingerprint(ref.k)}.${ext}`;
}
