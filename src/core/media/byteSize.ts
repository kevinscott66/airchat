...
export function formatByteSize(
  ...
  // v4.32.918: дробная часть отделяется запятой. В русском десятичный
  // разделитель — запятая, а здесь стояла точка, и это была единственная
  // дробь во всём приложении: «1.5 МБ» под фотографией, «52.4 МБ» в кэше,
  // «предел 1.8 МБ» в отказе. Рядом, в том же окне записи, соседний текст
  // предела был набран руками и уже писал запятую — «Тяжелее 1,2 МБ», — так
  // что два числа об одном и том же спорили друг с другом о разделителе.
  return `${String(shown).replace('.', ',')} ${unit.suffix}`;
}
