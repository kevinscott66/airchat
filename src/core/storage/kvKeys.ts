...
export const PRIVACY_PREF_KEYS = [
  ...
  // v4.32.807: ходить ли самому по чужой ссылке ради карточки предпросмотра.
  // Имя ключа начиналось с `privacy_`, а лежал он общим на устройство — то
  // есть второй аккаунт, заведённый ровно затем, чтобы его не связали с
  // первым, наследовал разрешение отдавать свой IP-адрес всякому, кто
  // пришлёт ссылку. Это решение об аккаунте, а не о телефоне: маяк опознаёт
  // не устройство, а собеседника, которому пишут.
  'privacy_link_preview_incoming',
] as const;
