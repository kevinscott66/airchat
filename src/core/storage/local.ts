...
/**
 * Забыть, что сервер уже видел эти записи (v4.32.595, переписано v4.32.615).
 *
 * Нужно ровно в одном случае: серверная копия аккаунта заведена заново.
 * Голова говорит две разные вещи сразу, и после потери серверной копии
 * ложной становится только одна из них:
 *
 *  • `fingerprint` — «ровно это содержимое там уже лежит». Ложно, сбрасываем:
 *    сборщик отправки снова выпишет запись целиком.
 *  • `revision` — «до какого номера эта запись дошла». Это НЕ утверждение о
 *    сервере, а местный счётчик, и на нём держится защита от отката:
 *    `applyMutation` отвергает пришедшее с номером не выше известного.
 *
 * До v4.32.615 здесь стояло `DELETE`, то есть терялось и второе. Смена метки
 * `serverEpoch` полностью в руках сервера, ответ с ней ничем не подписан — и
 * сервер, отдавший новую метку вместе со СТАРЫМИ мутациями (копия поднята из
 * вчерашнего снимка, а то и злонамеренно), возвращал переписку на день
 * назад: местных номеров, которыми это отбивается, уже не было. Теперь
 * номера остаются на месте — они ничего не стоят и ничему не мешают, а
 * следующая отправка просто продолжит счёт с них.
 *
 * Надгробия (`deleted = 1`) тоже остаются: заново их слать некому и незачем
 * — на новой серверной копии удалять нечего, — а номер удалённой записи
 * ровно так же отбивает её воскрешение старой мутацией.
 */
export async function forgetSyncEntityFingerprints(ownerProfileId: number): Promise<void> {
  if (!validSyncProfileId(ownerProfileId)) throw new Error('Invalid sync profile id');
  await (await db()).runAsync(
    'UPDATE sync_entity_heads SET fingerprint = NULL WHERE owner_profile_id = ?',
    [ownerProfileId],
  );
}
...
export type OutboxItem = {
  ...
  kind: string;
  /**
   * Содержимое конверта — или null, если столбец не открылся ключом устройства.
   *
   * v4.32.955: раньше тут стояла просто строка, а непрочитанный столбец
   * приезжал сюда пустым — неотличимо от честно пустого конверта. Разбор
   * очереди принимал пустоту за испорченную строку и УДАЛЯЛ её, засчитав в
   * «отправлено». Отдельное значение для «не прочитали» заставляет каждое
   * место, которое трогает нагрузку, сказать, что оно делает в этом случае.
   */
  payload: string | null;
  createdAt: number;
  priority: number;
 ...
 *
 * v4.32.500. Потолка у выборки не было. Каждая строка расшифровывается,
 * то есть весь ответ живёт в памяти открытым текстом, и
...
export async function outboxDrain(
  ...
  try {
    ...
    return rows.map((r) => ({
      id: r.id,
      kind: r.kind,
      // v4.32.955: не `decryptAtRestString` — он на отказе отдаёт пустую
      // строку, и очередь отправки теряла из-за этого сообщения насовсем
      // (см. разбор в sync.ts). «Не прочитали» тут обязано быть отличимо.
      payload: tryDecryptAtRest(r.payload, dek),
      createdAt: r.created_at,
...
/**
 * Тот же состав, что и `listGroupMembers`, но отличающий «участников нет» от
 * «прочитать не вышло» (v4.32.648).
 *
 * Пустой список — законный ответ: в группе может не остаться никого, кроме
 * тебя. На нём же строятся два вывода, которых неудавшееся чтение делать не
 * вправе: «этот отправитель не участник» и «рассылать некому». Оба ведут к
 * потере — сообщения живого участника и конверта, который никто не получит.
 * Кому исход важен, берёт эту обёртку; кому нужен перебор для показа —
 * прежнюю.
 */
export async function listGroupMembersRead(
  groupId: string,
  ownerProfileId: number
): Promise<GroupMemberRow[] | null> {
  return (await readGroupMembers(groupId, ownerProfileId))?.slice() ?? null;
}
