...
describe('мастер-ключ между экземплярами модуля', () => {
  ...
  it('стресс: двадцать прогонов по шесть вкладок — ни одна запись не теряется', async () => {
    ...
    // Двадцать прогонов по шесть вкладок — это 120 выводов мастер-ключа, около
    // 3,5 с на свободной машине. Умолчание jest (5 с) даёт запас в треть, и в
    // полном прогоне рядом с шестью сотнями наборов его не хватает: набор падал
    // по таймауту, ничего не сломав. Срок ставим явно — проверка тут про «ни
    // одна запись не теряется», а не про скорость.
  }, 30_000);
