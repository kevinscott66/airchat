...
describe('перечень шифруемых колонок', () => {
  ...
  it('группа: название, описание, черновик, превью, закреплённое, токен, аватар и адрес', () => {
    ...
    expect([...columnsOf('groups')].sort()).toEqual([
      ...
      'name',
      'pinned_message_text',
      // v4.32.681: username — публичный адрес группы. Он и правда публичный,
      // но перечень каналов, куда ходит владелец устройства, — нет.
      'username',
    ]);
  });
  ...
  it('альбомы историй: название альбома, подпись и адрес копии', () => {
    ...
    // то, где лежат снимки. media_cid — общий адрес снимка в IPFS: он уезжает
    // в синхронизацию, и по нему альбом собирается на втором устройстве, так
    // что открытым текстом он выдал бы содержимое альбома и без ключа.
    expect([...columnsOf('story_albums')]).toEqual(['title']);
    expect([...columnsOf('story_album_items')].sort()).toEqual(['media_cid', 'media_file', 'text']);
  });
});
