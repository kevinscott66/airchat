 ...
 * перед перезагрузкой.
 *
 * v4.32.581. Занятый файл перестал быть поводом не запуститься: вкладки делят
 * одно соединение (см. dbLease), и вторая вкладка работает через первую. Ветка
 * про занятый файл осталась ровно для одного случая — рядом открыта вкладка со
 * старой версией, которая в дележе не участвует и забирает файл себе. Поэтому
 * и совет теперь другой: не «закройте остальные вкладки», а «обновите их».
 */
...
const LOCKED_HOW_TO_FIX =
  'Обновите остальные вкладки с AirChat — и эту тоже. Если вкладка одна, значит ' +
  'файл держит не до конца закрытая прежняя: закройте браузер целиком — на ' +
  'iPhone смахните Safari из списка запущенных — и откройте AirChat заново. ' +
...
export function diagnoseStorageFailure(message: string, env: StorageEnv): string | null {
  ...
  if (locked === 'exact') {
    return (
      `Файл базы занят вкладкой со старой версией AirChat${where}. Вкладки давно ` +
      'делят базу между собой, но так умеют только версии 4.32.581 и новее — ' +
      'открытая рядом старая забирает файл себе целиком. ' +
      LOCKED_HOW_TO_FIX
    );
  }
  if (locked === 'vague') {
    return (
      ...
      'всегда это значит, что файл занят вкладкой со старой версией AirChat: ' +
      'вкладки делят базу между собой начиная с 4.32.581, а открытая рядом ' +
      'старая забирает файл себе целиком. ' +
      LOCKED_HOW_TO_FIX
    );
  }
