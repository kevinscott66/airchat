...
/**
 * Столбцы, которые ходят в копию и в синхронизацию, — и ровно они же
 * возвращаются `sanitizeRawChatMessageRows`.
 *
 * v4.32.692. Список нужен потому, что выгрузка брала строку целиком
 * (`SELECT *`), а запись обратно перечисляет только эти одиннадцать. Разница
 * — пять столбцов, добавленных миграциями позже (edited_at, reactions,
 * forwarded_from, starred, transport), и трогать их при записи нельзя: они
 * местные, и v4.32.615 специально закрыла их от перезаписи чужой ревизией.
 *
 * Пока выгрузка отдавала и их, синхронизация считала отпечаток по строке,
 * которую получатель заведомо не мог у себя воспроизвести: у него эти пять
 * оставались пустыми. Отпечаток расходился с сохранённой головой, получатель
 * отправлял «свою» версию обратно, у отправителя она ничего не меняла — и
 * следующий проход повторял всё сначала. Обмен шёл по кругу вечно, по одной
 * мутации на сообщение за проход, съедая предел MAX_PUSH_MUTATIONS.
 *
 * Соседние таблицы так и живут с самого начала: у переписок список записан
 * константой CONVERSATION_META_COLUMNS, у групп — своим модулем. Здесь строка
 * была последней, кто выгружался целиком.
 */
export const RAW_CHAT_MESSAGE_COLUMNS = [
  'id',
  'contact_pub_b64',
  'cid',
  'text',
