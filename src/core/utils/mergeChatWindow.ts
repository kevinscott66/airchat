 ...
 *
 * v4.32.653. Пустое `windowRows` означает «выборка с начала переписки не дала
 * ни строки», то есть в базе не осталось ничего, — и экран очищается. Отсюда
 * договор для вызывающего: сюда нельзя передавать результат чтения, которое
 * НЕ УДАЛОСЬ. Пока экран чата звал `getMessages`, гасящий сбой пустым списком,
 * одно неудачное открытие базы стирало всю переписку; теперь он читает окно
 * через `readMessageWindow`, у которого отказ виден в типе, и на отказе
 * склейка просто не запускается.
 */
