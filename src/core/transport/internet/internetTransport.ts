...
// v4.32.611 — правка по прямому запросу: сообщение должно дойти, даже если
// человек не заходил месяц. Снят заголовок `Cache: no` (см. send).
 ...
 *  - Срок хранения: 12 часов на публичном ntfy.sh, 30 суток на своём сервере
 *    (v4.32.611). Дольше этого срока сообщение теряется; feed-очередь на
 *    стороне отправителя делает retry при reconnect (v4.32.67
 *    networkReconnectWatcher + lanCoordinator.onPeerDiscovered).
 *
...
export class InternetTransport {
  ...
  async canReach(_targetDid: string): Promise<boolean> {
    if (!this.active) return false;
    // Любой контакт достижим, если у нас есть интернет. Relay сам хранит
    // сообщение до срока хранения — получатель подхватит при следующем
    // подключении к WS.
    try {
   ...
   * но relay хранит сообщение до срока хранения, так что fire-and-forget с
   * гарантией persistence.
   */
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        headers: {
          ...
          'X-Airchat-V': '1',
          Priority: 'default',
          // v4.32.611. Здесь стоял `Cache: 'no'` — с самого первого коммита.
          // Этот заголовок велит ntfy НЕ сохранять сообщение вообще: оно
          // доходило только до сокета, открытого прямо сейчас. То есть
          // persistence, на которую опирается весь механизм backlog
          // (relayBacklog, v4.32.545), не работала ни разу: закрыл приложение —
          // сообщение потеряно. Проверено на живом ntfy.sh: публикация с этим
          // заголовком не возвращается по `?since=`, без него — возвращается.
          //
          // Ценой снятия relay начинает хранить у себя шифротекст (12 часов на
          // ntfy.sh, 30 суток на своём сервере). Читать его relay всё равно не
          // может: конверт подписан или зашифрован до отправки, а тема
          // выведена из sha256(DID).
          Tags: 'airchat',
        },
        body,
  ...
  private openWs(): void {
    ...
    //
    // v4.32.545: было жёстко `10m`. Relay хранит накопленное, а забирали
    // десять минут — всё, что старше, терялось, хотя лежало на relay и ждало.
    // Человек, закрывший приложение на ночь, наутро не получал ни сообщений,
    // ни публикаций: и те и другие едут одним topic'ом. Теперь значение
    // приходит от координатора и считается от отметки последнего принятого
    // кадра (relayBacklog.sinceParam); при её отсутствии — вся глубина.
    //
