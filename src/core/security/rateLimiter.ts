...
/**
 * Что человек подтверждает, нажимая «Заблокировать» (v4.32.916).
 *
 * Обещание жило в трёх видах на четыре кнопки: карточка обещала «Сообщения и
 * звонки», чат — только «Сообщения», а два пункта в контактах не спрашивали
 * вовсе. Ни одна формулировка не была полной: запрет двусторонний, и своё к
 * заблокированному тоже перестаёт уходить — посты и истории отсеиваются по
 * тому же isBlocked (feedService, storyService, feedTransport), а звонок от
 * него отбивается молча (callService). Текст здесь один на все места, и
 * перечисляет он ровно то, что делает код.
 */
export const BLOCK_CONFIRM_TITLE = 'Заблокировать?';
export const BLOCK_CONFIRM_BODY =
  'Сообщения и звонки от этого человека будут отклонены. Ваши новые посты и истории к нему тоже не уйдут: блокировка двусторонняя.';
