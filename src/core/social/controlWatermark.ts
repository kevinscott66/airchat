 ...
 * (v4.32.793, {@link dmPinWatermarkKey}), для реакций — в v4.32.796
 * ({@link reactionWatermarkKey}): состояние реакции скалярно не на пару, а на
 * тройку «автор + сообщение + эмодзи», и знак сделан на неё.
 *
...
/**
 * Слот ОДНОЙ реакции (v4.32.796).
 *
 * Реакция тоже переключатель, и положение названо в самом кадре (`on`):
 * `toggleReaction` получает не «переключи», а готовое значение. Значит
 * сохранённый кадр возвращает снятую реакцию, а сохранённое «снять» убирает
 * поставленную, — и так столько раз, сколько кадр подадут. Тема relay
 * выводится из открытых DID, конверт живёт тридцать суток. Бьёт по каждому
 * получателю отдельно: у всех остальных под сообщением своя, верная картина,
 * поэтому расхождения не видно ни автору реакции, ни жертве.
 *
 * Ячейка на тройку «автор + эмодзи + сообщение». Ни одну часть выбросить
 * нельзя: реакций у одного человека под одним сообщением несколько, сообщений
 * много, а порядка между ними relay не держит — общий знак на собеседника
 * выбросил бы законное «👍 на сообщение B», пришедшее следом за более поздним
 * «❤️ на A».
 *
 * Порядок частей — от ограниченного к неограниченному. Открытый ключ в base64
 * двоеточий не содержит и берётся из ПОДПИСАННОГО отправителя. Эмодзи кодек
 * ограничивает восемью кодовыми точками, и каждая обязана попасть в
 * эмодзи-диапазоны — двоеточия (U+003A) среди них нет, так что разделитель
 * туда не вписать. Идентификатор сообщения ограничен только длиной — он и
 * последний.
 */
export function reactionWatermarkKey(
  authorPubB64: string,
  emoji: string,
  msgId: string
): string {
  return `${WATERMARK_PREFIX}react:${authorPubB64}:${emoji}:${msgId}`;
}

/** Свежесть реакции — без сдвига отметки (см. controlTsFresh). */
export async function reactionTsFresh(
  authorPubB64: string,
  emoji: string,
  msgId: string,
  pid: number,
  ...
  return freshTs(reactionWatermarkKey(authorPubB64, emoji, msgId), 'react', pid, ts);
}

/** Сдвинуть отметку реакции — после того, как она записана. */
export async function commitReactionTs(
  authorPubB64: string,
  emoji: string,
  msgId: string,
  pid: number,
  ...
  await commitTs(reactionWatermarkKey(authorPubB64, emoji, msgId), 'react', pid, ts);
}
