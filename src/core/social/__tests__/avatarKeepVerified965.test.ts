...
describe('форма исходников: неудача загрузки ничего не переписывает', () => {
  it('отступаются на любом пустом ответе, а не только когда нечего терять', () => {
    // В v4.32.972 к этому `continue` добавилась строка `retryLater`: отступаем
    // так же молча, но возвращаемся через минуту, а не через полчаса. Условие
    // отступления — прежнее, и закрепка здесь именно про него.
    expect(SRC).toContain('if (uri === null) {');
    expect(SRC).toContain('uri === null');
