...
describe('обе выдачи поиска строят плашку одним решением', () => {
  ...
  it.each(screens)('%s помечает пометку цветом предупреждения', (_name, read) => {
    // Кегль здесь не проверяется по числу: v4.32.528 увёл его в токен
    // `font.xs`, а утверждение теста — про ЦВЕТ предупреждения, не про размер.
    // Держать в нём вписанное число значило бы ронять тест на правке шрифта,
    // к которой он никакого отношения не имеет.
    expect(read()).toContain('color: colors.warning }}');
  });
