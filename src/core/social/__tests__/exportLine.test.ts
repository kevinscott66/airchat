...
describe('модуль остаётся чистым', () => {
  it('импортируются только такие же чистые соседи', () => {
    // До v4.32.806 импортов не было вовсе, и держалось это ради одного:
    // тело строки должно считаться без хранилища, транспорта и рендера.
    // Смысл тот же, но список типов сообщений обязан быть общим с подписью —
    // вторая копия разошлась бы с первой, как разошлись четыре копии до
    // v4.32.238. Поэтому пришпилен не ноль, а откуда именно разрешено брать.
    const from = moduleSrc()
      .split('\n')
      .filter((l) => /^import\s/.test(l))
      .map((l) => l.replace(/^.*from '([^']+)';$/, '$1'));
    expect(from).toEqual(['./forwardEnvelope', './messagePreview', './sysLineGuard']);
  });
});
