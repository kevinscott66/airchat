...
describe('pollVoteSync — куда положили и когда сняли', () => {
  ...
  it('снятый с полки голос обратно не ложится', () => {
    ...
    // v4.32.797: обратно голос ложится РОВНО в одном случае — «сейчас не
    // смогли». Прежде здесь стояло «ни одной ветки, кладущей голос обратно», и
    // это было верно для причины «сообщения нет» (она с приходом опроса ушла
    // навсегда), но заодно хоронило отложенный голос: снять с полки его уже
    // сняли, а перезапросить нечем. Ветка одна, она в repark, и зовут её
    // только из разбора отсрочки.
    expect(flush.match(/pendingVotes\.park/g) ?? []).toHaveLength(1);
    expect(flush).toContain('const repark = (v: ParkedVote): void => {');
    expect(flush).toMatch(/if \(intake === 'deferred'\) \{\n\s+failed \+= 1;\n\s+repark\(v\);/);
  });
...
describe('приём сообщения разгружает полку', () => {
  ...
  it('групповой приём зовёт снятие после записи строки', () => {
    ...
    // выдаётся за повтор.
    // v4.32.775: и той же операцией кладётся след в списке переписок, иначе
    // отказ следа терялся между двумя записями.
    const at = s.indexOf('await insertGroupMessageWithTouch(row, {');
    expect(at).toBeGreaterThan(0);
