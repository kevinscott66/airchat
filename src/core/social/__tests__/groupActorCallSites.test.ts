...
describe('вызывающие подключены к дому', () => {
  it('lookupGroupActor зовут все, кому нужна связка «группа + роль»', () => {
    const users = FILES.filter((f) => f.key !== HOME)
      // Обе формы — и схлопывающая, и различающая: дом у них один.
    ...
    // Приёмники конвертов оставались на прежней — им пустой ответ безобиден,
    // конверт просто не применится и отправитель повторит.
    //
    // v4.32.748: у отметки о прочтении в группе повтора нет — следующая
    // расскажет уже про следующее сообщение. Её приёмник перешёл на
    // различающую форму и откладывает конверт, вместо того чтобы объявить
    // участника посторонним по нечитаемому составу.
    //
    // v4.32.754: та же оговорка накрыла и реакцию. «Отправитель повторит» —
    // это про отправителя, который узнал об отказе; у реакции он не узнаёт
    // ничего: конверт разошёлся, экран показал реакцию, и повтора у
    // служебного конверта нет. Её приёмник тоже перешёл на различающую форму.
    // Осталось двое схлопывающих: голос и завершение опроса.
    const reading = outside.filter((l) => l.includes('await lookupGroupActorRead(')).length;
    expect(collapsing).toBe(2);
