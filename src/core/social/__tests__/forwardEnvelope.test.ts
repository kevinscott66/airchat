...
describe('пересылка мультимедиа не тащит машинное тело', () => {
  ...
  // v4.32.678: раньше здесь стояло обратное ожидание — «префикс снимает
  // экран». Снимал его только экран лички, а экран групп рисовал сырую строку
  // «\tvo:подпись»; и в обоих случаях подпись одноразового оседала у третьего
  // лица навсегда, хотя searchableText вырезает её из индекса именно затем,
  // чтобы она не пережила показ.
  it('одноразовое едет подписью, а не текстом автора', () => {
    const fwd = makeForwardText('Аня', '\x09vo:секретная подпись');
    expect(fwd).not.toContain('секретная подпись');
    expect(parseForwardedMessage(fwd)?.originalText).toBe('🔥 Одноразовое сообщение');
  });

  it('чужой клиент вложил одноразовое в пересылку вручную — разбор всё равно подменяет', () => {
    const p = parseForwardedMessage(`${FORWARD_PREFIX}Аня\n\x09vo:секретная подпись`);
    expect(p?.originalText).toBe('🔥 Одноразовое сообщение');
    expect(p?.originalText).not.toContain('секретная');
  });
