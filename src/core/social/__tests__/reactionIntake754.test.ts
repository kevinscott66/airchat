 ...
 * Откладывается ровно то, что пройдёт само.
 */
 ...
 * Метку такая версия не заклинит: приёмник (`internetCoordinator`, `failedOnce`)
 * даёт кадру ровно одну вторую попытку и после неё отпускает сам. Но каждый
 * заведомо мёртвый кадр она разбирает дважды, и в журнал ложится
 * `internet_frame_deferred_again` — строка, по которой ищут настоящие отсрочки.
 * Слово `deferred` стоит ровно столько, сколько значит. Пять проверок ниже
 * падают именно на такой версии.
 */
