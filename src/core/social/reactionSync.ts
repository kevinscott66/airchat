 ...
 * повтором не лечатся, и откладывать их незачем: приёмник даёт кадру ровно одну
 * вторую попытку (`internetCoordinator`, `failedOnce`), после чего отпускает
 * его сам. То есть «отложить всё подряд» метку не заклинит — но каждый заведомо
 * мёртвый кадр будет разобран дважды, а в журнале осядет
 * `internet_frame_deferred_again`, за которым перестанут видеть настоящие
 * отсрочки. Слово должно значить то, что значит.
 */
export async function handleIncomingReaction(
  ...
  if (!res.ok) {
    ...
    // v4.32.754: 'failed' — это упавший запрос, и он пройдёт сам.
    //
    // v4.32.821: и 'missing' тоже. Раньше «строки сообщения нет» считалось
    // постоянным наравне с потолками и нечитаемым столбцом, а это гонка, а не
    // приговор. Кадры пачки разбираются параллельно — транспорт зовёт onFrame
    // на каждое сообщение WS, не дожидаясь предыдущего, — и реакция успевает
    // дойти до SELECT раньше, чем рядом допишется само сообщение. Своего часа
    // это ждёт ровно там, где чаще всего и случается: человек был offline,
    // relay при подключении отдаёт сообщение и реакцию на него одной пачкой.
    // Реакция при этом терялась насовсем: повтора у служебного конверта нет,
    // а собеседник видит её у себя и уверен, что она стоит у обоих.
    //
    // Лишним ожиданием это не обернётся: реакция на сообщение, которого у нас
    // не будет никогда (прислали раньше, чем я вступил в группу; своё я
    // удалил), стоит одного повторного разбора — приёмник даёт кадру ровно
    // вторую попытку и после неё отпускает метку сам (internetCoordinator,
    // failedOnce). Потолки и нечитаемый столбец остаются постоянными: их
    // вторая попытка не изменит.
