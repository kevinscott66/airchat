...
async function materialize(
  ...
  for (const { pubB64, urlKey, version } of pending) {
    ...
    try {
      ...
      // v4.32.965: не достали или не проверили — это не «фото сняли». Условие
      // здесь снималось только когда показывать и так было нечего; а если
      // прежнее проверенное лицо на экране было, его затирало пустотой. То
      // есть связь моргнула ровно в ту секунду, когда человек сменил фото, — и
      // вместо прежнего лица остаётся буква в кружке, хуже, чем до смены.
      // Снятое фото сюда не попадает: у него версии нет вовсе, и разбирается
      // оно выше, в справочнике.
      //
