...
/**
 * Сколько всего весит запись — текст, фото и документы вместе (v4.32.919).
 *
 * Тот же конверт на 2 МБ, только целиком: берётся 90 % от потолка — запас на
 * подпись и JSON-обвязку. Прежде это число жило локальной переменной внутри
 * publishFeedPost, а отказ на экране называл предел словами, набранными руками
 * в ru.json. Теперь предел один и на проверку, и на подпись под ней.
 */
export const FEED_POST_MAX_BYTES = Math.floor(2 * 1024 * 1024 * 0.9);
