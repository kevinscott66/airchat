 ...
 * своим контактам.
 *
 * v4.32.942: снятый возраст безопасен только потому, что вид конверта теперь
 * ограничен там, откуда кадр берётся. Приёмный путь разбирает девять видов, и
 * бессрочно годный конверт любого из них — это «выполни у себя то, что назовёт
 * сервер». Список видов держится в `publicPost.ts` рядом с проверкой подписи:
 * оба условия про один и тот же ответ сервера, и разносить их нельзя.
 */
