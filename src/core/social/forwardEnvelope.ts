 ...
 * «🎤 Голосовое сообщение».
 *
 * v4.32.678: сюда дописано '\x09vo:' (одноразовое). Раньше его тело ехало
 * как есть — «его снимет экран при отрисовке». Снимал его только экран лички
 * (ChatScreen), а экран групп разбирает пересылку отдельной веткой и своей
 * проверки на '\x09vo:' там нет: пересланное в группу одноразовое рисовалось
 * сырой строкой «\tvo:подпись». Но главное не разметка: подпись одноразового
 * во всём остальном коде считается тем, чему не место в постоянной истории —
 * searchableText вырезает её из индекса целиком «чтобы не находить через
 * месяц то, от чего режим защищает», а messagePreview показывает вместо неё
 * подпись. Пересылка была единственной дверью, через которую подпись
 * отправителя оседала у третьего лица навсегда — и в личке (там её и правда
 * рисовали текстом), и в поиске получателя.
 */
const MACHINE_PREFIXES = [
  ...
  '\x06doc:',
  '\x07loc:',
  '\x09vo:', // одноразовое — в пересылку едет подпись, а не текст автора
