...
/**
 * v4.32.671: просьба прислать свой профиль.
 *
 * До этой версии профиль ездил только «от себя»: рассылкой по контактам после
 * правки и досылкой при открытии переписки. То есть карточка собеседника
 * показывала юзернейм, «О себе», местоимения и статус ровно у тех, кто сам
 * открывал приложение и наш с ним чат. У всех прочих — включая тех, кому уже
 * написан не один десяток сообщений, — она оставалась пустой сколько угодно
 * долго, и отличить «у него не задан юзернейм» от «его карточка до нас не
 * доехала» было нельзя ни человеку, ни коду.
 *
 * Тело у просьбы пустое: единственное, что в ней есть по смыслу, — кто просит,
 * а подписанный отправитель известен приёму и без тела. Отвечает profileSync,
 * и отвечает с ограничением по частоте: ответ — это отправка своей карточки по
 * чужой команде.
 *
 * Байт тот же, что у профиля: это тот же разговор, а слово после байта разное.
 */
export const PROFILE_REQ_PREFIX = '\x14preq:';

/** Просьба прислать профиль. Тело не разбирается — его нет. */
export function isProfileRequest(text: string): boolean {
  return text.startsWith(PROFILE_REQ_PREFIX);
}

/** Текст просьбы. Отдельной функцией, чтобы префикс не писали руками. */
export function encodeProfileRequest(): string {
  return PROFILE_REQ_PREFIX;
}
