 ...
 *
 * Успех здесь — «конверт ушёл в сеть», а не «вызов sendMessage состоялся»
 * (v4.32.713). sendMessage отдаёт идентификатор при отправке и null при отказе:
 * контакт заблокирован, исчерпан часовой лимит служебных конвертов, нет общего
 * ключа, нет маршрута в сеть. Ни один из этих null не означает «положено в
 * очередь и уйдёт позже»: очереди повторной отправки у служебного конверта нет
 * вовсе — outboxEnqueue из продакшена не вызывается, а строки в chat_messages
 * служебный конверт не заводит, так что и находить сборщику нечего. Поэтому
 * null равен «адресат не узнает никогда» и считается отказом наравне с
 * исключением: считать его принятым значит промолчать ровно в том случае,
 * ради которого весь этот итог и заведён.
 */
...
export async function fanoutControlEnvelope(
  ...
  await Promise.allSettled(
    recipients.map(async (pub) => {
      try {
        const cid = await svc.sendMessage(pub, payload);
        if (cid) {
          accepted += 1;
          return;
        }
        log.warn('control_fanout_send_refused', { op, to: pub.slice(0, 12) });
      } catch (e) {
