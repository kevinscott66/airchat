...
export class MessagingService {
  ...
  /**
   * Разобрать входящий конверт — а не сумев, вернуть его памяти повторов
   * (v4.32.790).
   *
   * Дефект. Отметка `seenMessageIds` ставится ДО разбора: у конверта три
   * входа (общая тема, свой ящик, локальная сеть), и разбирать его трижды
   * нельзя. Дальше по тексту разбора восемь мест отвечают `'deferred'` —
   * «сейчас не смогли, перезапросите» — и ещё четырнадцать могут ответить
   * тем же словом из обработчиков групп, историй, реакций, голосов,
   * закреплений, исчезающих, профилей. Отметку не снимало ни одно из них.
   * Снимали её только выходы `'consumed'` выше по тексту — те, где разбор
   * осознанно отказывался от конверта.
   *
   * Цена. Единственная повторная попытка, которую даёт координатор интернета,
   * входит сюда снова, натыкается на отметку первой же строкой и отвечает
   * `'consumed'` — молча, без записи в журнале. Метка «докуда прочитано»
   * перешагивает кадр, `giveUp('internet_frame_deferred_again')` не
   * срабатывает никогда, и всё отложенное пропадает навсегда: сообщения,
   * реакции, закрепления, таймеры исчезающих, профили, истории, голоса,
   * групповые сообщения, отметки о прочтении, заявки на вступление, баны и
   * переименования. Отправитель при этом видит «Доставлено». То есть весь
   * механизм отсрочки для личного транспорта не работал.
   *
   * Тот же вход по CID (`receiveCid`) ответ выбрасывает сознательно, в
   * расчёте на следующий заход по тому же CID, — и этот заход упирался в
   * отметку.
   *
   * Правка. Снятие отметки связано с самим словом `'deferred'`, а не с
   * местом, где оно произносится: разбор целиком обёрнут, и ни одна новая
   * ветка отсрочки больше не может об этом забыть. Образец — ленты
   * (`feedService`), где `feedSeenForget` зовётся перед каждой отсрочкой
   * руками; там таких пар четыре, и следить за ними приходится глазами.
   *
  ...
  private async persistIncomingFromEnvelopeInner(
    ...
    //
    // v4.32.767: исход записи — слово, а не молчание, и на этот вопрос отвечает
    // сама запись. Раньше здесь стояло отдельное чтение строки ДО неё, и оно
    // сплющивало «строки нет» с «базу не удалось спросить» (v4.32.763 оставляла
    ...
    const alreadyStored = stored === 'duplicate';
