...
export async function handleIncomingGroupControl(text: string, rcpt: GroupRecipient, senderPubB64?: string): Promise<EnvelopeIntake> {
  ...
  if (env.op === 'join') {
     ...
     *
     * v4.32.819: пересказ идёт ДО записи состава, и его отказ откладывает кадр.
     *
     * Прежде он звался через `void` последним действием — и рассылка, не
     * ушедшая никому (служба переписки не поднята), пропадала бесследно: кадр
     * уже объявлен разобранным, повторов у управляющего конверта нет, а второй
     * попытки не делает никто. Десять человек из тридцати так и не узнавали о
     * новичке — ровно та немота, ради ухода от которой пересказ и заведён.
     *
     ...
     * «устарел». Поэтому порядок перевёрнут — сначала рассказать, потом
     * записать. Цена перестановки мала и не нова: если разбор оборвётся между
     * рассылкой и записью, остальные узнают о новичке раньше меня, а повтор
     * кадра допишет его и мне. Лишний пересказ безвреден по той же
     * идемпотентности, что и пересказ второго администратора.
     *
     * Пустой список адресатов (в группе кроме нас никого) — это удача, а не
     * отказ: fanoutControlEnvelope отвечает `sent: true`.
     */
    if (iAmAdmin) {
      ...
      if (myPubAdd) {
        ...
        const toldAdd = await sendGroupControlTo(
          members
        ...
        if (!toldAdd.sent) {
          log.warn('group_ctl_join_relay_undelivered', {
            gid: env.groupId.slice(0, 8),
            from: senderPubB64.slice(0, 12),
            reason: toldAdd.reason,
          });
          return 'deferred';
        }
      }
  ...
  if (env.op === 'pin') {
    ...
    const write = await applyLocalPin({ groupId: env.groupId, ownerProfileId: pid, msgId: env.msgId, on: env.on });
    // v4.32.643: записи не было — и строки «Сообщение закреплено» быть не
    // должно: она сказала бы о том, чего в шапке нет.
    //
    // v4.32.758: и кадр в таком случае откладываем. Оба отказа здесь — заминка
    // базы, она пройдёт сама; «разобрано» же двигает метку докуда прочитано, а
    // relay отдаёт накопленное только по ней — то есть занятая на секунду база
    // стоила закрепления навсегда, при том что у остальных участников оно есть.
    if (!write.ok) {
