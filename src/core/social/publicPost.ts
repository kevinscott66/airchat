 ...
 * а только после того, как автор явно подтвердил «Опубликовать по ссылке» —
 * отдельным пунктом меню либо в том же диалоге перед «Копировать ссылку» и
 * «Поделиться» (AC-04, см. postLinkFlow). Отозвать её можно тем же меню.
 *
...
/**
 * Что вообще может лежать в публичном хранилище (v4.32.942).
 *
 * Тот же список, что сервер проверяет на выкладке: публикация и репост. Копия
 * по ссылке — это запись, которую можно прочитать, а не действие, которое надо
 * выполнить. Держится он здесь, а не только на сервере, потому что ответ
 * сервера — не источник правды; см. `getPublicPostFrame`.
 */
const PUBLIC_POST_TYPES: readonly string[] = ['feed_post', 'feed_repost'];
...
export async function getPublicPostFrame(postId: string): Promise<Uint8Array | null> {
  ...
  try {
    ...
    // v4.32.942: подпись и номер сходятся — а конверт всё ещё может быть не
    // публикацией. Кадр отсюда уходит на общий приёмный путь ленты, а тот
    // разбирает девять видов конвертов, и «открыть ссылку» превращалось в
    // «выполнить у себя то действие, которое назовёт сервер».
    //
    // Дороже всего обходился `feed_delete` — подписанный автором, настоящий, и
    // до v4.32.941 автор отдавал такой серверу при каждом отзыве ссылки. Пост
    // по ссылке открывают как раз тогда, когда его на устройстве нет, поэтому
    // ветка удаления клала надгробие «на будущее», а `postWriteGuard` потом
    // отказывался принимать ту же публикацию, приехавшую от автора по сети.
    // Отзыв ссылки — не удаление записи, но у прочитавшего она пропадала
    // навсегда, и починить это у себя он не мог ничем.
    //
    // (Надгробие от ЧУЖОГО ключа безвредно с v4.32.615: ключ таблицы составной,
    // и чужая строка настоящую публикацию не глушит. Опасен именно авторский.)
    if (typeof parsed.type !== 'string' || !PUBLIC_POST_TYPES.includes(parsed.type)) {
      log.warn('public_post_type_refused', {
        postId: postId.slice(0, 24),
        type: typeof parsed.type === 'string' ? parsed.type.slice(0, 32) : 'none',
      });
      return null;
    }
