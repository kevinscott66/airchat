...
/** Живая геолокация живёт максимум 8 часов. */
export const LIVELOC_MAX_AHEAD_MS = 8 * 60 * 60_000;
/**
 * Допуск в минуту на округление у чужих клиентов (v4.32.688: сравнивается с
 * концом сессии, то есть с числом от тех же часов, а не с часами получателя).
 */
...
export function parseLiveLoc(text: string, now: number = Date.now()): LiveLocPayload | null {
  ...
  // v4.32.688: потолок времени отправки — конец сессии, а не «сейчас».
  //
  // Прежняя граница (потолком служил `now + LIVELOC_MAX_SKEW_MS`) сравнивала число,
  // написанное часами ОТПРАВИТЕЛЯ, с часами ПОЛУЧАТЕЛЯ. На телефоне, чьи часы
  // отстают больше чем на минуту, `ts` отбрасывался у КАЖДОЙ посылки — и
  // молча уносил с собой обе вещи, ради которых заведён в v4.32.563: подпись
  // «Геолокация не обновляется» (без `ts` остаётся прежнее «жива до срока»)
  // и защиту от посылки, подсунутой назад (`decideLiveLocUpdate` без времени
  // применяет любую). Отстающие часы возвращали пузырю ровно тот обман,
  // который та версия чинила: «идёт» над замершей точкой.
  //
  // Сама проверка нужна: `ts` на век вперёд заморозил бы карту — все
  // следующие посылки сессии стали бы «старее» и отбрасывались, а плашка
  // LIVE горела бы дальше. Но потолком ему служит конец его же сессии:
  // рассылка не шлёт такт после срока (`sentAt >= expireAt` в
  // liveLocationService), а сам `expireAt` уже ограничен восемью часами выше.
  // Оба числа теперь от одних часов, и расхождение с чужими на решение не
  // влияет; минута допуска — на округление у других клиентов.
  //
  // Слишком старое время не отбрасывается: оно честно означает, что рассылка
  // замерла.
