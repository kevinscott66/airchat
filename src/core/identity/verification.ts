 ...
 *
 * Привязка к `did` — это и есть привязка к идентификатору аккаунта: ссылка
 * `AC-XXXXX-XXXXX` целиком выводится из ключа (см. accountRef), поэтому
 * галочка, выданная на ключ, действует ровно на один аккаунт и ни на какой
 * другой. Обратный порядок был бы слабее: ссылка — это 50 бит хэша, и связать
 * бумагу с ней означало бы принять любой аккаунт, чей хэш подобрали. Поэтому
 * `ref` в нагрузке необязателен и служит только сверкой: он читается глазами
 * в журнале выдачи, а если он есть — обязан совпасть с тем, что даёт ключ.
 *
 * К устройству галочка НЕ привязана намеренно. Аккаунт восстанавливается на
 * новом телефоне из сид-фразы, и бумага, выданная на udid, после переустановки
 * приложения превратилась бы в бумагу на чужое имя — а второй телефон того же
 * человека остался бы без галочки при том же самом ключе.
 *
...
export async function readGrant(
  ...
  for (const keyB64 of OFFICIAL_VERIFIER_KEYS) {
    ...
    // Необязательная сверка ссылки на аккаунт. Записана в бумаге — значит,
    // выдающий имел в виду именно этот аккаунт и назвал его дважды; расхождение
    // означает ошибку выдачи, и молча предпочесть одну половину нельзя.
    if (payload.ref !== undefined && payload.ref !== publicIdFor('account', did)) return null;
    return claim;
  }
  return null;
}
