...
export function checkUsernameClaim(value: unknown, unlocked?: unknown): UsernameClaim {
  ...
  // v4.32.936: нижняя граница протокола — абсолютная, её не открывает и
  // бумага. Стояла она здесь только последним словом, через `normalizeUsername`
  // в конце, и потому давала причину `charset`: человеку говорили про
  // недопустимые символы там, где дело было в длине. Хуже, что на сервере её
  // не было вовсе — имя из одной буквы, выписанное бумагой, ложилось в реестр
  // и оставалось там навсегда недостижимым: все пути разбора «@имени» в
  // приложении проходят через `normalizeUsername`, который его не пропустит.
  if (raw.length < USERNAME_MIN) return { ok: false, reason: 'too_short' };
