...
export type GlassSurfaceProps = ViewProps & {
  ...
  rim?: boolean;
  /**
   * Положить в стекло водяной знак «AirChat».
   *
   * v4.32.545: знак жил в полосе статуса поверх всего — то есть был не водяным
   * знаком, а восьмой строкой интерфейса рядом с часами. Место ему в шапке,
   * под самой капсулой Dynamic Island: в работе он не мозолит глаза, а на
   * снимке экрана остаётся.
   *
   * v4.32.573: знак рисовался РАНЬШЕ `BlurView` — то есть уходил размытию в
   * подложку. На устройстве от него не осталось ничего: размытие силой 52
   * растирает восемнадцать точек высоты по всей капсуле, и «водяной знак под
   * стеклом» на деле означало «водяного знака нет». Теперь под размытием
   * лежит только подкрас — мягкость и есть его смысл, — а буквы идут поверх
   * стекла и остаются водяным знаком по непрозрачности, а не по тому, что их
   * стёрли. Содержимое шапки рисуется после них и всегда сверху.
   *
 ...
 * Знак с v4.32.573 идёт отдельно и поверх стекла: пятну размытие на пользу,
 * буквам — на уничтожение.
 *
  ...
  return (
    ...
    >
      {/* Подкрас — ДО BlurView: он ложится размытию в подложку, и мягкость у
          ...
          вовсе — прятать не за что, и ни подкрас, ни знак не рисуются: на
          глухой заливке они оказались бы чёткими и заметнее всего именно там,
          где просили меньше. */}
      {watermark && !solid ? (
      ...
      {/* Буквы — ПОСЛЕ BlurView, иначе размытие растирает их в ничто (см. проп
          `watermark`). Водяным знаком их держит непрозрачность, а содержимое
          шапки рисуется следом и перекрывает их там, где они встретились. */}
      {watermark && !solid ? (
        <View pointerEvents="none" style={styles.watermark}>
          <AirChatWordmark height={18} style={styles.watermarkGlyph} />
        </View>
      ) : null}
      {rim ? (
        <>
