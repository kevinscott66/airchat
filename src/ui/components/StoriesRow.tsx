                  ...
                  Alert.alert('Сторис', undefined, [
                    {
                      ...
                      style: 'destructive',
                      onPress: () => {
                        // v4.32.908: удаление шло сразу, первым пунктом меню и
                        // в один палец. Вернуть сторис нечем — строка и файл
                        // снимка стираются, — а конверт уже разослан личными
                        // сообщениями каждому контакту (см. storyService): у
                        // них копия останется, пока не истечёт сама. Про это
                        // человеку надо сказать ДО, а не выяснять потом.
                        Alert.alert(
                          'Удалить сторис?',
                          ...
                          [
                            { text: 'Отмена', style: 'cancel' },
                            {
                              ...
                              style: 'destructive',
                              onPress: () => {
                                // v4.32.625: у обещания не было .catch.
                                // deleteStory ходит в базу и сносит файл
                                // снимка — отказ там возможен, и тогда onClose
                                // не звался: просмотрщик оставался открытым с
                                // той же сторис, без единого слова о том, что
                                // удаление не прошло.
