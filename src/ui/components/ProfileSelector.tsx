...
export function ProfileSelector({
  ...
  const confirmDelete = (profile: Profile): void => {
    Alert.alert(
      'Удалить профиль?',
      // v4.32.731: прежний текст обещал обратное тому, что происходит, —
      // «локальные данные останутся в базе, но ключ сменится». На деле ключ
      // как раз и остаётся (он выводится из seed-фразы по номеру профиля и
      // вернётся, если завести профиль снова), а данные удаляются все: строки
      // базы профиля, копия диалогов, файлы аватара и историй, черновик поста.
      // Согласие, полученное под неверным описанием, — не согласие, а на
      // необратимом действии это стоит человеку всей его переписки.
      `«${profile.name}» и все его данные на этом устройстве: переписка, группы, ` +
        'истории, лента и очередь отправки. Вернуть их будет нельзя.',
      [
        { text: 'Отмена', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              try {
                // v4.32.731: ответ читается. `false` значит «не удалил» —
                // список профилей недоступен либо этой строки в нём уже нет, —
                // и раньше в обоих случаях показывалось «Профиль удалён».
                // v4.32.741: ответ стал различающим. «Удалил» раньше значило
                // только «вычеркнул строку»: данные могли целиком остаться на
                // устройстве, и человек узнавал об этом никогда.
                ...
                if (!result.removed) {
                  showError(
                    result.reason === 'cleanup_failed'
                      ? 'Не удалось стереть данные профиля — он остался на месте. Закройте другие окна приложения и попробуйте ещё раз'
                      : result.reason === 'switch_failed'
                        ? 'Не удалось перейти на другой профиль, поэтому удаление не начиналось. Данные остались на месте'
                        : 'Не удалось удалить профиль. Данные остались на месте');
                  return;
                }
                // v4.32.849: строка профиля — тоже остаток, и не такой, как
                // файлы: данные стёрты, а запись о профиле на устройстве
                // осталась прежней. После перезапуска он вернётся в список
                // пустым, и сказать об этом надо здесь — иначе человек
                // решит, что удаление отменилось само.
                if (result.leftovers.includes('row')) {
                  showError('Данные профиля стёрты, но из списка он не убрался — запись на устройстве не переписалась. После перезапуска он вернётся пустым: удалите его ещё раз');
                  return;
                }
                if (result.leftovers.length > 0) {
                  // Строки профиля больше нет, но часть файлов осталась. Это не
                  // успех и не отказ, и выдавать одно за другое нельзя:
                  // фотографии удалённого аккаунта лежат на устройстве.
