 ...
 *
 * Роль и имя для озвучки задаёт вызывающий: `accessibilityRole` и
 * `accessibilityLabel` проходят насквозь (на вебе react-native-web делает из
 * них `role` и `aria-label`). По умолчанию роли НЕТ, и это намеренно: обёрткой
 * сделаны и кнопки, и строки списков, и вкладки, и переключатели, и подложки
 * модалок — «кнопка» на каждой из них соврала бы озвучке не реже, чем молчание.
 * Без роли на вебе нажимаемое остаётся фокусируемым `div` без имени, поэтому
 * у основных действий (вход, заведение и восстановление аккаунта, пароль)
 * роль и подпись обязательны — их держит onboardingA11y.test.tsx.
 */
