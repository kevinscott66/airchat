...
describe('разделы содержимого', () => {
  ...
  // v4.32.575: разделов ровно четыре. Файлы, музыка, голосовые и ссылки
  // были отдельными плашками, хотя открывали ту же галерею — просто сразу
  // на своей вкладке. Вкладки внутри «Медиа» остались, полоса перестала
  // повторять их собой.
  it('состав разделов совпадает с тем, что карточка умеет открыть', () => {
    expect(ids(hubSections(base)))
      .toEqual(['wall', 'stories', 'media', 'starred']);
  });
  ...
  it('у владельца аккаунта архив идёт после общих разделов', () => {
    expect(ids(hubSections({ ...base, isSelf: true })))
      .toEqual(['wall', 'stories', 'media', 'starred', 'archive']);
  });
});
