...
describe('ПОВОД ДЛЯ ПРАВКИ ЖИВ', () => {
  ...
  it('счётчик и список посмотревших показываются именно автору', () => {
    ...
    // Слово рядом с числом склоняется (4.32.899), поэтому здесь закреплено
    // только само число: важно, что счётчик берётся из viewers.length.
    expect(src).toMatch(/👁 Просмотрело \$\{viewers\.length\}/);
  });
