  ...
  const handleGrpRestoreDeleted = useCallback((entry: GroupRecentlyDeletedEntry) => {
    void (async () => {
      try {
        ...
        // v4.32.892: у этого `insertGroupMessage` ответ выбрасывался, а ниже
        // запись вычёркивалась из корзины безусловно. `insertGroupMessage` при
        // отказе базы не бросает — он отдаёт `false` (см. `GroupMessageWrite`),
        // так что `catch` ниже сюда не приходил: человеку писали «Сообщение
        // восстановлено», корзину чистили, ленту перечитывали — и сообщения не
        // было нигде. Корзина здесь последняя копия, и это единственное место в
        // приложении, где промах уничтожает текст насовсем: в личных чатах
        // восстановление только кладёт текст обратно в поле ввода. `id` строки
        // выдан только что, повтор невозможен — `false` значит ровно «не
        // записалось».
