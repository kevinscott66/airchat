        ...
        >
          <View style={styles.modalOverlay}>
            {/* v4.32.122: на Android используем flex:1 + marginTop=insets.top+8 — модалка
                ...
                v4.32.540 переводил iOS с открытой клавиатурой на то же правило
                через флаг `keyboardVisible`. Правило верное, флаг — нет: он
                приходит из `keyboardWillShow`, а отступ снизу KeyboardAvoidingView
                берёт из `keyboardWillChangeFrame`, и там, где событие одно без
                другого, лист оставался свёрнутым в уже урезанном контейнере и
                верхом уходил за кромку экрана.

                v4.32.604: высота листа флагом больше не выбирается. Форма у него
                одна — 0.72 экрана снизу, — а ограничивает её `maxHeight: '100%'`,
                то есть внутренняя область контейнера. Клавиатуру из неё вычитает
                сам KeyboardAvoidingView, чёлку — его `paddingTop`; отдельного
                состояния для этого не нужно. */}
            <KeyboardAvoidingView
              style={[
                styles.modalKeyboardAvoid,
                // v4.32.604: верхняя кромка вычитается здесь, а не маржином листа.
                // Тогда «сколько осталось от экрана» знает контейнер — и знает
                // целиком: клавиатуру он вычитает сам (`behavior="padding"`),
                // чёлку вычитает этот отступ.
                Platform.OS === 'ios' ? { paddingTop: insets.top + 8 } : null,
              ]}
                ...
                style={[
                  ...
                  Platform.OS === 'ios'
                    ? {
                        ...
                        // v4.32.604: выше остатка лист не бывает. Процент берётся
                        // от внутренней области контейнера, из которой клавиатура
                        // и чёлка уже вычтены, поэтому ограничение не зависит от
                        // того, успел ли флаг клавиатуры обновиться. Раньше высота
                        // выбиралась флагом `keyboardVisible`, и при открытой
                        // клавиатуре лист оставался свёрнутым (0.72 экрана) внутри
                        // контейнера, которому KeyboardAvoidingView уже отнял снизу
                        // всю клавиатуру: 671 pt листа в 580 pt области при
                        // `justify-content: flex-end` — это верх на −91 pt, то есть
                        // «Отмена», заголовок и «Опубликовать» за верхней кромкой.
                        maxHeight: '100%',
