  ...
  const handleToggleBiometric = useCallback((next: boolean) => {
    if (!next) {
      ...
      void disableBiometricUnlock()
        .then((removed) => {
          // Переключатель гаснет в любом случае: признак снимается первым и без
          // системного запроса, то есть Face ID действительно больше не входит.
          // Отказ здесь означает другое — сам пароль остался лежать в хранилище
          // устройства, а именно от этого человек и отказывался (v4.32.810).
          setBioEnabled(false);
          if (!removed) {
