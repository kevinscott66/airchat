...
function ContactsScreenImpl({ onOpenChatWithPeer, pair, myDid }: Props): React.ReactElement {
  ...
  const submitAdd = useCallback(async () => {
    ...
    // v4.32.910: имя по умолчанию собиралось своей формой — шесть знаков
    // base64 с головы и без многоточия, — тогда как везде в приложении тот же
    // человек подписан `shortIdentity`. Голова и двусторонняя подпись не
    // пересекаются, и «Контакт sYk1v0» рядом с «sYk1v0…1qA4E=» человек не
    // сведёт; вдобавок шесть знаков без многоточия читаются как законченное
    // слово, а не как обрезанный ключ.
    const name =
      addNameInput.trim() || `Контакт ${shortIdentity(Buffer.from(parsedKey).toString('base64'))}`;
