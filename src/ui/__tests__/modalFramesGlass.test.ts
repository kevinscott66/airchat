...
describe('лист «Новой публикации» и клавиатура', () => {
  ...
  it('ограничение — сжатием, а не процентным maxHeight', () => {
    // С `maxHeight: '100%'` Yoga урезал высоту листа, но позицию при
    // `justify-content: flex-end` считал от несжатой: при открытой клавиатуре
    // шапка уходила за верхнюю кромку, а над клавиатурой оставалась пустота.
    expect(sheet()).not.toMatch(/maxHeight:\s*'100%'/);
  });
