    ...
    void (async () => {
      try {
        ...
        deferAfterFirstFrame(async () => {
          ...
          // v4.32.587. Звонки поднимаются ПЕРВЫМИ и НЕ в общей очереди.
          //
          // Регистрация на сигнальном сервере — единственное, от чего зависит
          // приём входящего звонка: без неё сервер про телефон не знает и
          // отвечает звонящему `peer_unavailable`. Стояла она последней в
          // цепочке из десятка `await` — за транспортами LAN и интернета,
          // синхронизацией аккаунта, лентой и уборкой кэша. Ни у одного из
          // этих ожиданий нет срока: достаточно, чтобы одно из них зависло на
          // плохой сети, и телефон навсегда оставался невидимым для звонков,
          // ничем это не показывая. Принимать звонки — не фоновая уборка, и
          // ждать своей очереди за ней оно не должно.
          void initCallService(pair, pid).catch((e: unknown) => {
    ...
    */}
    ...
    >
      ...
      */}
      ...
      {/*
        v4.32.555: штамп лежит ПОД «островом» — и в этом весь его смысл.
...
function AppShell({
  ...
  return (
    <View style={styles.root}>
      ...
      <OfflineStatus />
      {children}
      {/* v4.32.610: «Соединение…»/«Обновление…» объясняет, почему очередь
          отправки стоит на месте. Последней — капсула лежит поверх экрана под
          строкой состояния и не сдвигает его (см. ConnectionStatus). */}
      <ConnectionStatus />
    </View>
  );
}
