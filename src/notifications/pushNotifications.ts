...
export class PushNotificationService {
   ...
   *
   * v4.32.981: и токен сверяется тоже — тем же условием, каким сверяется
   * пришедший ответ (см. `registerTokenTracked`). Токена в проверке не было, а
   * `registerTokenTracked` первой же строкой объявляет свой токен текущим —
   * поэтому повтор, сработавший пока новый токен ещё едет к ретранслятору,
   * задним числом делал своим мёртвый токен. Дальше по кругу: ответ о мёртвом
   * приходил «своевременным» и признавался записанным, а ответ о живом —
   * устаревшим и отбрасывался. У ретранслятора остаётся последняя запись,
   * то есть мёртвая.
   *
   * Случай не выдуманный: токен обновляют как раз тогда, когда старый умер, —
   * а значит его запись в эти же секунды и срывается, заводя повтор. Первая
   * ступень лестницы (5 с) короче срока одного обращения к ретранслятору
   * (до 10 с), так что повтор попадает внутрь этого окна легко.
   */
