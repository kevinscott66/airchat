 ...
 *
 * v4.32.941: снятие копии больше не требует конверта `feed_delete`. Требовать
 * его было дорого: «Отозвать ссылку» отдавало серверу подписанный приказ убить
 * эту же запись у каждого, кому она придёт, — приказ бессрочный и годный к
 * повтору, хотя саму запись автор удалять не просил. Намерения достаточно:
 * оно названо этим постом, действием `del` и разовым числом, которое гасится
 * тут же. Старый путь с конвертом сервер по-прежнему принимает — с ним ходят
 * сборки до v4.32.941, — и первый тест держит его живым.
 */
...
test('публичная копия публикации кладётся, читается и снимается', async (t) => {
  ...
  // AC-04: подписанный feed_delete служит и «Отозвать ссылку» — сама запись у
  // автора остаётся. Отзыв не навсегда: опубликовать по ссылке снова можно, и
  // копия снова открывается. (С v4.32.941 приложение ходит сюда без конверта —
  // см. следующий тест; здесь держится совместимость со старыми сборками.)
